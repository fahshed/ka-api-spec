...
export interface IKAApiClent {
     ...
     * @return OK
     */
    doneFollowAdvice(cancelToken?: CancelToken | undefined, onDownloadProgress?: (progressEvent: ProgressEvent<EventTarget>) => void): Promise<ErrorMessage>;
    /**
...
export class KAApiClent extends AuthorizedApiBase implements IKAApiClent {
     ...
     */
    doneFollowAdvice(cancelToken?: CancelToken | undefined, onDownloadProgress?: (progressEvent: ProgressEvent<EventTarget>) => void): Promise<ErrorMessage> {
        let url_ = this.baseUrl + "/user/advice/followed";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.processDoneFollowAdvice(_response);
        });
    }

    protected processDoneFollowAdvice(response: AxiosResponse): Promise<ErrorMessage> {
        const status = response.status;
        let _headers: any = {};
        if (response.headers && typeof response.headers === "object") {
            for (let k in response.headers) {
